...
class AttentionExpertMixture(nn.Module):
    ...
    def _balance_loss(self, probs: torch.Tensor) -> torch.Tensor:
        ...
        u = torch.full_like(q, 1.0 / probs.size(-1))
        # Keep the old KL-to-uniform objective as the main stabilizer, but
        # only push on large batch-level deviations outside the configured
        # tolerance. This allows genuinely useful experts to dominate without
        # letting routing collapse completely.
        kl = torch.sum(q * torch.log(q.clamp_min(safe_eps(q.dtype)) / u))
        deviation = torch.abs(q - u)
        excess = F.relu(deviation - self.balance_tolerance)
        tolerance_penalty = torch.mean(excess * excess)
