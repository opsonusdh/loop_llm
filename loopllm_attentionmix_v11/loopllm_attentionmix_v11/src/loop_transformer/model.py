...
class LoopTransformer(nn.Module):
    ...
    def update_routing_biases(self) -> None:
        """Apply each FFN routing block's loss-free balance correction once.

        FeedForward owns the persistent activation/expert bias buffers and the
        actual correction rule. The model-level method is the single integration
        point used by the trainer after an optimizer step, so every block updates
        consistently and the buffers are included in checkpoints.
        """
        for block in self.blocks:
            update = getattr(block.ffn, "_update_routing_biases", None)
    ...
    def forward_adaptive_exit(
        ...
        *,
        ...
        """Run recurrent loops sequentially and stop each example at exit.

        Unlike ``forward(..., max_loops)`` followed by selecting one returned
        loop, this method performs real early-exit computation: once an example
        satisfies the Q-exit threshold, it is removed from subsequent recurrent
        loop computation. This makes the Stage-II gate an actual compute-saving
        mechanism rather than a post-hoc selector.

        Returns
        -------
        chosen_logits : [B,T,V]
            The logits from the loop selected for each example.
        selected_loops : [B]
            1-based selected loop depth for each example.
        exit_cdf : [B]
            Cumulative exit probability at the selected loop.
        """
        ...
        blocks: List[torch.Tensor] = [active_embedding]
        loop_states: List[torch.Tensor] = []
        survival = torch.ones(B, device=device, dtype=embedding.dtype)
        cdf = torch.zeros(B, device=device, dtype=embedding.dtype)
        selected_loops = torch.full((B,), self.cfg.max_loops, device=device, dtype=torch.long)
        selected_logits: List[Optional[torch.Tensor]] = [None] * B

        for loop_idx in range(self.cfg.max_loops):
            partial = self._one_loop(blocks, loop_idx=loop_idx)
            if self.recurrent_depth_controller is not None and loop_states:
                partial = self.recurrent_depth_controller(
                    partial, loop_states[-1], loop_states[0], loop_idx
                )
            loop_states.append(partial)
            blocks.append(partial)
            ...
            h_normed = self.final_norm(h_out)
            pre_logits = (
                self.embed_proj_out(h_normed)
                if self.embed_proj_out is not None
                else h_normed
            )
            logits = self.lm_head(pre_logits)
            lam = self.exit_gate(h_normed, loop_idx=loop_idx)

            if loop_idx < self.cfg.max_loops - 1:
                p_exit = lam * survival[active]
                survival_active = survival[active] * (1.0 - lam)
            else:
                p_exit = survival[active]
                survival_active = survival[active]

            cdf[active] = cdf[active] + p_exit
            hit = cdf[active] >= exit_threshold

            if loop_idx == self.cfg.max_loops - 1:
                hit = torch.ones_like(hit, dtype=torch.bool)

            if hit.any():
                hit_positions = torch.nonzero(hit, as_tuple=False).flatten()
                hit_global = active[hit_positions]
                hit_logits = logits[hit_positions]
                selected_loops[hit_global] = loop_idx + 1
                for local_pos, global_pos in zip(hit_positions.tolist(), hit_global.tolist()):
                    selected_logits[global_pos] = hit_logits[local_pos]

            keep = ~hit
            if not keep.any():
                break

            # Carry only unresolved examples into the next loop. This is what
            # turns the gate into real compute savings for heterogeneous batches.
            active = active[keep]
            active_idx = active_idx[keep]
            blocks = [b[keep] for b in blocks]
            loop_states = [state[keep] for state in loop_states]
            survival = survival.clone()
            survival[active] = survival_active[keep]
            cdf = cdf.clone()

        if any(x is None for x in selected_logits):
            raise RuntimeError("Adaptive exit failed to assign logits to every example.")

        chosen = torch.stack([x[:, :] for x in selected_logits if x is not None], dim=0)
        return chosen, selected_loops, cdf
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            if n_loops is not None:
                loop_logits, _ = self.forward(idx, run_loops)
                chosen = loop_logits[-1]
            else:
                # Real early exit: only unresolved examples continue through
                # deeper loops. This is the path that can actually save
                # recurrent inference compute.
                chosen, _, _ = self.forward_adaptive_exit(
                    idx, exit_threshold=exit_threshold
                )
