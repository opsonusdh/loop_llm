...
def main() -> int:
    ...
    p.add_argument("--token-ids", default=None, help="comma-separated integer token IDs; avoids requiring tiktoken")
    ...
    if args.token_ids is not None:
        try:
            values = [int(x.strip()) for x in args.token_ids.split(",") if x.strip()]
    ...
    print("\n--- GENERATED ---")
    if enc is not None:
        print(enc.decode(ids[0].detach().cpu().tolist()))
    else:
        print("token_ids:", ids[0].detach().cpu().tolist())
    return 0
