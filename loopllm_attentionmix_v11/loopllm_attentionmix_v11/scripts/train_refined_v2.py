...
def estimate_loss(model: LoopTransformer, data: np.memmap, batch_size: int,
    ...
    for _ in range(eval_iters):
        ...
        # Validation must measure the objective named by training_mode.
        # A hybrid run still needs its recurrent CE reported separately;
        # merely enabling diffusion_blocks must not silently replace that
        # metric with the much larger sigma-weighted denoising loss.
        if model.cfg.training_mode == "diffusion":
            loss, _ = model.diffusion_blocks_loss(batch)
        else:
            loss, _ = model.compute_loss(batch, max_loops=model.cfg.max_loops)
...
def build_config(args: argparse.Namespace) -> LoopConfig:
    cfg_dict = {}
    args._json_training_mode_explicit = False
    if args.config_json:
        with open(args.config_json) as f:
            json_cfg = json.load(f)
        cfg_dict.update(json_cfg)
        args._json_training_mode_explicit = "training_mode" in json_cfg
    ...
    cli_fields = {
        ...
        "activation_bias_update_speed": args.activation_bias_update_speed,
...
def parse_args() -> argparse.Namespace:
    ...
    # DiffusionBlocks recurrent-depth training.
    p.add_argument("--diffusion-blocks", action="store_true", default=None,
    ...
    p.add_argument("--activation-bias-update-speed", type=float, default=None,
                    help="non-gradient load-balancing nudge speed for activation routing")
    p.add_argument("--moe-bias-update-speed", type=float, default=None,
                    help="non-gradient load-balancing nudge speed for MoE routing")
...
def main() -> None:
    ...
    if (
        cfg.diffusion_blocks
        and args.training_mode is None
        and not getattr(args, "_json_training_mode_explicit", False)
        and cfg.training_mode == "recurrent"
    ):
    ...
    if args.resume and ckpt_path.exists():
        ...
        ckpt = load_checkpoint(ckpt_path, device=device, restore_rng=True)
        model = ckpt["model"]
        ...
        if "optimizer_state_dict" not in ckpt:
            raise RuntimeError(
                f"Resume checkpoint {ckpt_path} has no optimizer_state_dict; "
                "use --init-from for a weights-only restart."
            )
        optimizer.load_state_dict(ckpt["optimizer_state_dict"])
        log.info(f"Resumed at step {start_step} with optimizer attached to checkpoint model")
    elif args.resume:
        log.warning(
    ...
    for step in range(start_step, args.max_steps):
        ...
        # Loss-free routing-bias correction is part of the model's training
        # algorithm, not an optional diagnostic. FeedForward owns the update
        # rule; LoopTransformer applies it to every block immediately after the
        # gradient-based optimizer step so the correction is checkpointed with
        # the model and remains effective on resumed runs.
