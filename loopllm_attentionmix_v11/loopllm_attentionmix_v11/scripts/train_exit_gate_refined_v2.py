...
def main():
    ...
    if a.resume:
        rng_extra = ck.get('extra', {}).get('gate_rng_state', {}) if 'ck' in locals() else {}
        if rng_extra:
            if 'train_generator' in rng_extra:
                g.set_state(rng_extra['train_generator'])
            if 'val_generator' in rng_extra:
                vg.set_state(rng_extra['val_generator'])
    def gate_extra(best_value):
        return {
            'stage': 'stage2_exit_gate',
            'base_checkpoint': str(a.checkpoint),
            'best_val_bce': best_value,
            'sharpness': a.sharpness,
            'margin': a.margin,
            'gate_rng_state': {
                'train_generator': g.get_state(),
                'val_generator': vg.get_state(),
            },
        }
    ...
    for step in range(start,a.max_steps+1):
        ...
        if step%a.eval_interval==0 or step==a.max_steps:
            ...
            if best is None or metrics['gate_bce'] < best:
                ...
                save_checkpoint(out/'best.pt',model,opt,step,extra=gate_extra(best))
        if step%a.checkpoint_interval==0 or step==a.max_steps:
            save_checkpoint(latest,model,opt,step,extra=gate_extra(best))
