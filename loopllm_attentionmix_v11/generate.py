...
def overlay_trained_gate(base_model: torch.nn.Module, gate_model: torch.nn.Module) -> None:
    """Copy all exit_gate.* tensors from Stage-II into the matching Stage-I model."""
    base_state = base_model.state_dict()
    gate_state = gate_model.state_dict()

    gate_keys = [
        key for key in gate_state
        if key.startswith("exit_gate.")
    ]
    if not gate_keys:
        raise RuntimeError("Gate checkpoint contains no exit_gate parameters.")

    base_non_gate_digests = {
        key: tensor_digest(value)
        for key, value in base_state.items()
        if not key.startswith("exit_gate.")
    }
...
def select_loop(
    ...
    """Per-example Q-exit selection matching LoopTransformer.generate()."""
    if not loop_logits:
        raise RuntimeError("Model returned no loop logits.")

    batch = loop_logits[0].shape[0]
    device = loop_logits[0].device
    chosen = loop_logits[-1].clone()
    chosen_depth = torch.full((batch,), len(loop_logits), dtype=torch.long, device=device)
    survival = torch.ones(batch, device=device, dtype=loop_logits[0].dtype)
    cdf = torch.zeros(batch, device=device, dtype=loop_logits[0].dtype)

    for t, (lgts, lam) in enumerate(zip(loop_logits, lambdas)):
        lam = lam.to(device=device, dtype=survival.dtype).reshape(-1)
        if lam.numel() == 1:
            lam = lam.expand(batch)
        if lam.numel() != batch:
            raise RuntimeError(
                f"Exit gate returned {lam.numel()} values for batch size {batch}."
            )
        ...
        if t == len(loop_logits) - 1:
            ...
            exit_prob = survival * lam
        cdf = cdf + exit_prob

        undecided = (chosen_depth == len(loop_logits)) & (cdf >= threshold)
        if undecided.any():
            chosen = torch.where(
                undecided[:, None, None],
                lgts,
            ...
            chosen_depth = torch.where(
                undecided,
                torch.full_like(chosen_depth, t + 1),
                chosen_depth,
            )
    ...
    return chosen, chosen_depth, cdf, survival
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Only exit_gate.* tensors (projection + loop embedding) are copied into the base model."
        ),
    )
    ...
    prompt_group.add_argument(
        "--token-ids",
        help="Comma-separated integer token IDs. Avoids requiring tiktoken.",
    )
...
def main() -> None:
    ...
    if args.loops is None and not args.gate_checkpoint:
        raise ValueError(
            "No --gate-checkpoint was supplied. Provide --loops N for fixed-loop generation."
        )
    ...
    if args.token_ids is not None:
        try:
            prompt_ids = [int(x.strip()) for x in args.token_ids.split(",") if x.strip()]
        except ValueError as exc:
            raise ValueError("--token-ids must be comma-separated integers") from exc
        if not prompt_ids:
            raise ValueError("--token-ids produced no IDs")
    else:
        try:
            import tiktoken  # type: ignore
        except ImportError as exc:
            raise RuntimeError(
                "Text prompts require tiktoken. Install it with `pip install tiktoken`, "
                "or use --token-ids to generate without the tokenizer package."
            ) from exc
    ...
    with torch.no_grad():
        for step in range(args.max_new_tokens):
            ...
            if args.loops is not None:
                loop_logits, lambdas = model.forward(idx, max_loops=args.loops)
                chosen = loop_logits[-1]
                chosen_loop = args.loops
                cdf = 1.0
                lambda_values = [float(x.mean().item()) for x in lambdas]
            else:
                chosen, chosen_depth, cdf_tensor = model.forward_adaptive_exit(
                    idx, exit_threshold=args.exit_threshold
                )
                chosen_loop = int(chosen_depth[0].item())
                cdf = float(cdf_tensor[0].item())
                lambda_values = []
            ...
            if args.debug:
                ...
                if lambda_values:
                    ...
                    print("[DEBUG] adaptive gate: sequential early-exit path")
                ...
                for rank, (token_id, prob) in enumerate(
                    ...
                    token_text = tokenizer.decode([token_id]) if tokenizer is not None else str(token_id)
                    print(
    ...
    if args.no_stream:
        print(tokenizer.decode(generated_ids) if tokenizer is not None else " ".join(map(str, generated_ids)), end="")
