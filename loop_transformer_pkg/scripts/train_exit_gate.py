...
def main() -> int:
    ...
    # Load to CPU first before moving to device to prevent DirectML type crash in torch.load
    LOG.info("Loading Stage-I checkpoint: %s", args.checkpoint)
    base_ckpt = load_checkpoint(args.checkpoint, device="cpu")
    model = base_ckpt["model"].to(device)
    model.eval()
    ...
    if args.resume:
        ...
        model = resume_ckpt["model"].to(device)
        model.eval()
    ...
    for step in range(start_step, args.max_steps + 1):
        ...
        if step == 1 or step % args.log_interval == 0 or step == args.max_steps+1:
            LOG.info(
        ...
        if step == 1 or step % args.log_interval == 0 or step == args.max_steps+1:
            time2 = time.time()
            LOG.info("Time passed: %s | Average time per step: %s", 
                    str(time2-start_time), str((time2-start_time)/((step+1) - start_step))
                )
            print()
