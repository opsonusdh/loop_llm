...
def gate_lambda_from_hidden(
    ...
    pooled = torch.stack([h.mean(dim=1) for h in hidden_states], dim=1)
    logits = exit_gate.proj(pooled).squeeze(-1)
...
def q_exit_depth_from_lambdas(
    ...
    """Mirror model.generate(): batch-level mean lambda + survival CDF."""
    if lambdas.ndim != 2:
        raise ValueError(f"Expected lambdas [B,T], got {tuple(lambdas.shape)}")
    mean_lambdas = lambdas.mean(dim=0)
    survival = 1.0
    cdf = 0.0
    chosen = lambdas.size(1)  # 1-based depth; fallback is max depth.
    for t in range(lambdas.size(1)):
        p_t = float(mean_lambdas[t].item()) * survival
        cdf += p_t
        if t < lambdas.size(1) - 1:
            survival *= 1.0 - float(mean_lambdas[t].item())
        if cdf >= threshold:
            chosen = t + 1
            break
    return chosen, cdf
...
def evaluate(
    ...
    *,
    ...
    for batch_index in range(eval_batches):
        ...
        # Faithful Q-exit: current model.generate uses a batch-level mean lambda.
        chosen_depth, _cdf = q_exit_depth_from_lambdas(lambdas, threshold)
        gated_loss = losses[:, chosen_depth - 1]
        ...
        selected_counts[chosen_depth - 1] += batch_size
        batch_depth_sum += chosen_depth * batch_size
        total_loop_computations += chosen_depth * batch_size
        ...
        if (batch_index + 1) == 1 or (batch_index + 1) % max(1, eval_batches // 5) == 0:
            LOG.info(
                "batch %d/%d | baseline=%.4f gated=%.4f depth=%d | elapsed=%.1fs",
                batch_index + 1,
                eval_batches,
