...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--loop-monotonic-weight", type=float, default=None,
                    help="soft penalty when a later loop's mean loss exceeds an earlier one's; 0 disables")
    p.add_argument("--loop-monotonic-margin", type=float, default=None,
                    help="allowed tolerance before the monotonic penalty kicks in (default 0.01)")
    p.add_argument("--activation-bias-update-speed", type=float, default=None,
                    help="loss-free-balancing step size for activation routing (arXiv:2408.15664); requires model.update_routing_biases() to be called, which it now is")
    p.add_argument("--moe-bias-update-speed", type=float, default=None,
                    help="loss-free-balancing step size for the MoE expert router")
