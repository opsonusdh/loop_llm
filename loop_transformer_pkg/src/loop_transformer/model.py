...
class LoopTransformer(nn.Module):
    def __init__(self, cfg: LoopConfig):
        ...
        self.exit_gate  = ExitGate(cfg.dim)
        ...
        self.last_loop_monotonic_penalty: Optional[torch.Tensor] = None
    ...
    def _one_loop_impl(self, loop_idx: int, *blocks_tuple: torch.Tensor) -> torch.Tensor:
        """
        ...
        for i, block in enumerate(self.blocks):
            ...
            attn_out = block.forward_attn(h_in, loop_idx=loop_idx)
            ...
            ffn_out = block.forward_ffn(h_in, loop_idx=loop_idx)
            partial = partial + ffn_out

        return partial  # type: ignore[return-value]

    def _one_loop(self, blocks: List[torch.Tensor], loop_idx: int = 0) -> torch.Tensor:
        """
        Run one loop pass, optionally wrapped in gradient checkpointing.

        With cfg.grad_checkpointing=True (and only while self.training),
        this recomputes the loop's sub-layer activations during backward
        instead of keeping them all resident, cutting peak activation
        memory roughly max_loops-fold at the cost of ~2x the forward
        compute for this portion. This is the "activation recomputation"
        the AttnRes paper (§3.1) describes as standard practice once
        depth-multiplied activation memory stops fitting in budget --
        exactly the situation LoopLM creates by construction, since every
        loop reruns the same N blocks and (without checkpointing) all
        max_loops copies of their activations must stay live for backward.
        """
        if self.cfg.grad_checkpointing and self.training:
            return grad_checkpoint(self._one_loop_impl, loop_idx, *blocks, use_reentrant=False)
        return self._one_loop_impl(loop_idx, *blocks)
    ...
    def forward(
        ...
        for loop_idx in range(max_loops):
            ...
            lambdas.append(self.exit_gate(h_normed))  # exit gate always sees the full-width representation
    ...
    def update_routing_biases(self) -> None:
        """Apply one loss-free-balancing step (arXiv:2408.15664) to every
        FeedForward layer's activation_bias/expert_bias, using each
        layer's most recent forward's hard-usage stats.

        Call ONCE PER OPTIMIZER STEP, after optimizer.step() -- e.g. in
        scripts/train.py's training loop:

            loss.backward()
            optimizer.step()
            model.update_routing_biases()   # <-- add this line
            optimizer.zero_grad()

        NOT wired into forward()/compute_loss() automatically: see
        FeedForward.update_routing_biases()'s docstring for why doing so
        would silently break under grad_checkpointing. Safe to leave
        uncalled -- activation_bias_update_speed / moe_bias_update_speed
        default to a small positive value, but each bias buffer stays at
        its zero-init until this is actually called, so skipping it just
        means pure auxiliary-loss balancing (the prior behavior), not an
        error.
        """
        for module in self.modules():
            if module is not self and hasattr(module, "_update_routing_biases"):
                module._update_routing_biases()
    ...
    def compute_loss(
        ...
        # Per-example minimum-improvement penalty. For each consecutive loop
        # pair, a "pass" requires the later loop to beat the earlier one's
        # loss by at least `loop_monotonic_margin`; a merely-neutral loop and
        # a catastrophically worse one both count as a full "fail" -- this is
        # a threshold-crossing signal, not a magnitude-scaled one. `previous`
        # is detached so the model can't satisfy the objective by making the
        # EARLIER loop worse instead of the later one better.
        #
        # A hard 0/1 indicator has zero gradient almost everywhere, so this
        # uses a straight-through estimator: the forward value is exactly
        # the hard penalty (surrogate - surrogate.detach() is identically 0
        # regardless of what `surrogate` is, so the choice of surrogate only
        # affects the backward pass, never the forward value). The surrogate
        # is softplus, not sigmoid: sigmoid((threshold-improvement)/T)'s
        # gradient is a bump that VANISHES on both sides of the threshold,
        # so at T=0.01 (checked empirically) any violation worse than about
        # -0.01 -- i.e. most of the range this penalty exists to correct --
        # gets essentially zero gradient, silently doing nothing for exactly
        # the examples that need it most. softplus's gradient (a sigmoid,
        # one derivative up) saturates to a CONSTANT near 1 for large
        # violations instead of decaying back to 0, so badly-violating
        # examples keep a real, non-vanishing gradient at any distance from
        # the threshold; it only fades out on the safe/already-passing side,
        # which is the side where fading out is correct.
        if (
            self.cfg.loop_monotonic_weight > 0.0
            and step_losses_per_example.size(1) > 1
        ):
            previous = step_losses_per_example[:, :-1].detach()
            current = step_losses_per_example[:, 1:]
            improvement = previous - current
            threshold = self.cfg.loop_monotonic_margin

            hard_penalty = (improvement < threshold).to(current.dtype)

            temperature = 0.01
            surrogate = F.softplus((threshold - improvement) / temperature)
            monotonic_penalty = (
                hard_penalty + surrogate - surrogate.detach()
            ).mean()
        else:
            monotonic_penalty = torch.zeros(
                (),
                device=step_losses_per_example.device,
                dtype=step_losses_per_example.dtype,
            )
        self.last_loop_monotonic_penalty = monotonic_penalty.detach()
        total_loss = total_loss + self.cfg.loop_monotonic_weight * monotonic_penalty
        ...
        return total_loss, step_losses_per_example.mean(dim=0).detach()  # [T]
