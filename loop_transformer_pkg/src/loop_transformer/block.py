...
class ExitGate(nn.Module):
    """
    Per-step instantaneous exit probability predictor (Ouro LoopLM,
    arXiv:2510.25741, §3.2).

    Instantaneous exit probability at loop step t:
        λ_t(x) = σ(Linear(mean-pool(h^(t))))  ∈ (0, 1)

    These are combined via the survival model to build a proper discrete
    distribution over exit steps (see LoopTransformer.compute_loss).
    """
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        """h: [B, T, D]  →  λ_t: [B]"""
        return torch.sigmoid(self.proj(h.mean(dim=1))).squeeze(-1)
